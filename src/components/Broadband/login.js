...
class Login extends Component {
  ...
  validateLogin = e => {
    this.setState({ isSubmitted: true, errorMessage: "" });
    e.preventDefault();
    var apiEndpointLogin = "";
    if (this.props.platform === "broadband_bt") {
      apiEndpointLogin =
        process.env.REACT_APP_API + "auth/createtoken?username=";
    } else {
      console.log("Error");
  ...
  doLogin() {
    insertLog(1, "Login", "Username: " + JSON.stringify(this.state.username));

    axios.defaults.headers.common["Authorization"] =
      "Bearer " + this.props.globalState.jwtAuth;
    axios
      .post(process.env.REACT_APP_API + "UserAccount/GetAgentDetails")
      .then(response => {
      ...
      .catch(error => {
        ...
        if (error.response.status === 401) {
          this.setState({
            errorMessage:
              "An error has occured, please try again in a few miniutes",
            isSubmitted: false
          });
        } else {
          this.setState({
