...
class PackageSummary extends Component {
    ...
    componentWillMount() {
        ...
        db.open().then(async function(){
          ...
          this.setState({
            ...
            CanHaveVirgin: this.props.globalState.isWeb ? true : customerServices.canHaveVirgin,
            Aerial:  this.props.globalState.isWeb ? true : customerServices.hasAerial
          })
          this.getQuote()
        })        
    }
    ...
    render() {
        this.media_featuresMarketing = this.state.media_features.filter(this.featuresMarketing).map((item, index) => {
            return (
                <div key={index}>
                    <ul>
                        <li>
                            <span data-fieldname="Description">
                                {item.Description}
                            </span>
                        </li>
                    </ul>
        ...
        this.media_featuresAddons = this.state.media_features.filter(this.featuresAddons).map((item, index) => {
            return (
                <div key={index}>
                    <ul>
                        <li>
            ...
            <div>
                ...
                { this.props.globalState.isMultiJourney ? 
                    <ul>
                        ...
                        <li className='current'><a href='#/'><span>Package summary</span></a></li>
                ...
                { this.props.globalState.isMultiJourney && 
                          <div>
                              <div className='userDetails'><strong>{this.state.title} {this.state.firstName} {this.state.lastName}</strong></div>
                              <div className='userDetails'><strong>Tel: {this.state.telNo}, Postcode: {this.state.Postcode}</strong></div>
                          </div>
                        }
                    <div className='perfect-package-wrapper summary'>
                        <div className='card-header-wrapper'>Your perfect package - summary</div>
                        <div className='logo-wrapper'><img src={this.LogoPath + '/imagesPackage/' + this.state.media_provider.ProviderLogo} alt='Logo' /></div>
                        <div className='perfect-package-inner'>
                            <div className='perfect-package-name'>{this.state.media_provider.ProviderName}<br />{this.state.data.PackageName}</div>
                            <div className='perfect-package-speed'><strong>{this.state.data.MaxSpeed} Mb/s </strong>maximum download speed</div>
                        </div>

                        <div className='perfect-package-summary'>
                            <div className='perfect-package-inner-summary'>
                                <p><span>Monthly cost:&nbsp;</span><span>&pound;{parseFloat(this.state.data.MonthlyCost).toFixed(2)}</span></p>
                                <p><span>Data Allowance:&nbsp;</span><span>{this.state.data.DataAllowanceDesc}</span></p>
                                <p><span>Contract Length:&nbsp;</span><span>{this.state.data.ContractLengthMonths} months</span></p>
                                <p><span>Fixed Price Months:&nbsp;</span><span>{this.state.data.FixedPriceMonths} months</span></p>
                                <p><span>Setup Fee:&nbsp;</span><span>&pound; {this.state.data.SetupFee ? this.state.data.SetupFee : 0}</span></p>
                            </div>
                        </div>
                    </div>

                    <div className='perfect-package-wrapper features' style={this.state.media_features.filter(this.featuresMarketing).length > 0 ? { display: 'block' } : { display: 'none' }}>
